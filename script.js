...
(function () {
  ...
  function initMusic() {
    ...
    const iconOn = btn.querySelector('.music-btn__icon--on');
    const iconOff = btn.querySelector('.music-btn__icon--off');
  ...
  function startMusic() {
    ...
    audio.play().then(() => {
      btn.classList.add('is-playing');
      btn.querySelector('.music-btn__icon--on').style.display = '';
      btn.querySelector('.music-btn__icon--off').style.display = 'none';
    }).catch(() => {});
  }
  ...
  function initCalendar() {
    ...
    // Naver Calendar link
    $('#naverCalBtn').href = 'https://calendar.naver.com';
  }
  ...
  function initStory(storyImages) {
    ...
    // 영상이 설정된 경우 영상을 section__inner 바깥(section.story)에 표시
    if (CONFIG.story.video) {
      const storySection = $('#story');
      const videoWrapper = document.createElement('div');
      videoWrapper.className = 'story__video-wrapper animate-item';
      videoWrapper.setAttribute('data-animate', 'fade-up');

      const url = CONFIG.story.video;
      const youtubeMatch = url.match(/(?:youtube\.com\/(?:watch\?v=|embed\/)|youtu\.be\/)([a-zA-Z0-9_-]{11})/);
      const vimeoMatch = url.match(/vimeo\.com\/(\d+)/);

      if (youtubeMatch) {
        videoWrapper.innerHTML = `<iframe class="story__video" src="https://www.youtube.com/embed/${youtubeMatch[1]}" frameborder="0" allowfullscreen allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture"></iframe>`;
      } else if (vimeoMatch) {
        videoWrapper.innerHTML = `<iframe class="story__video" src="https://player.vimeo.com/video/${vimeoMatch[1]}" frameborder="0" allowfullscreen></iframe>`;
      } else {
        videoWrapper.innerHTML = `<video class="story__video" src="${url}" controls playsinline preload="metadata"></video>`;
      }

      storySection.appendChild(videoWrapper);
      return;
    }
  ...
  function initRSVP() {
    const SCRIPT_URL = 'https://script.google.com/macros/s/AKfycbzExHImMvlUBwCtzn5VTmkZtsgV04vjG_7TCAiCafRxHrt7xM78yKNcwIzN1bPfYPFK/exec';
    const form = $('#rsvpForm');

    form.addEventListener('submit', async (e) => {
      e.preventDefault();
      const name = $('#rsvpName').value.trim();
      const attendance = form.querySelector('input[name="rsvpAttendance"]:checked')?.value;
      const side = form.querySelector('input[name="rsvpSide"]:checked')?.value;
      const message = $('#rsvpMessage').value.trim();

      if (!name || !attendance || !side) {
        showToast('이름과 참석 여부, 구분을 입력해주세요');
        return;
      }

      const btn = form.querySelector('.rsvp__submit');
      btn.disabled = true;
      ...
      try {
        await fetch(SCRIPT_URL, {
          ...
          body: JSON.stringify({ name, attendance, side, message })
        });
        showToast('전달되었습니다 🌿');
        form.reset();
      } catch {
  ...
  function initGuestbook() {
    ...
    function renderEntries() {
      ...
      if (entries.length === 0) {
        list.innerHTML = '<p class="guestbook__empty">아직 남겨진 메시지가 없습니다.</p>';
        return;
      }
      list.innerHTML = entries.slice().reverse().map(e => `
        <div class="guestbook__item">
          <div class="guestbook__item-header">
            <span class="guestbook__item-name">${e.name}</span>
            <span class="guestbook__item-date">${formatDate(e.ts)}</span>
          </div>
          <p class="guestbook__item-message">${e.message}</p>
        </div>
      `).join('');
    }
